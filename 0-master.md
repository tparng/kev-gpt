...
- `5-demo-prd.md`, the live demo product spec
- `6-past-the-stream-ceiling.md`, the speed campaign — what happened *after* the model was running, when the levers became cycles and clock rather than getting it on-chip at all
...
- **Where it stands on silicon (measured, not modelled).** The whole INT4 transformer runs in fabric with the CPU out of the loop — the stage-3 sequencer — generating a **token-stream-bit-exact** Kevin, deterministically. The headline as of this writing: **59,965.5 tok/s @ 200 MHz, MEASURED** (16-stream split-brain, 16/16 streams bit-exact, 3/3 runs), up from the first on-fabric run at **0.07** and the first CPU-out-of-the-loop sequencer at **44.3** — roughly **5,000× the optimised A53 wall (~11)** on the same model, and well past a laptop GPU. The climb went through a 2k rung (pipelined non-linears + wide lanes) to ~25k (batching N=16 streams through one resident-weight pass — the *stream ceiling*, provably the max at LANES=128), and then a second era where, streams maxed, the only levers left were **cycles and clock**: split-brain (two cohorts on the dual-ported URAM) and a systematic worst-path-retirement campaign took it 25,744.5 → 36,970.7 → 46,604.4 → 56,262.7 → 59,965.5. That second era is doc 6. The road on is the cycle floor (~53k cyc gated in sim → ~40k designed) times 250 MHz silicon — the **100k identity, 16 × 250 MHz / 40k cyc** — which is PROJECTED, not done. The full ladder, tiered measured / sim / projected, is `fabric/progress.png`.
