...
CFG
echo "layer=$LAYER M=$M K=$K (AXI BFM -> gemv_axi_load -> gemv_load)"
iverilog -g2012 -I "$SIM" -o "$SIM/sim.vvp" \
    fabric/stage2/rtl/gemv_load.sv fabric/stage2/rtl/gemv_axi_load.v fabric/stage2/tb/tb_axi_load.sv
vvp "$SIM/sim.vvp"
...
print(f"PYVERDICT bitexact={dut[:n]==gold[:n] and len(dut)==len(gold)} maxabserr={maxerr} n={n}")
PY
