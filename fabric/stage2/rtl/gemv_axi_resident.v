// -----------------------------------------------------------------------------
// gemv_axi_resident — AXI4-Lite shell around gemv_resident. Whole model loaded
// ONCE into URAM at startup; per layer the host just sets W_BASE/M/K, streams the
// activation, and runs. Adds W_BASE to the gemv_axi_load register map.
//
//   0x00 CTRL(b0 start,b1 ld_rst,b2 reset) 04 STATUS 08 M_COUNT 0C K_COUNT
//   10 W_DATA(load stream) 14 X_DATA 18 RD_ADDR 1C RD_DATA 20 CYCLES
//   24 IDCODE(0x4745_4D52 "GEMR") 28 W_BASE(byte offset of layer in URAM)
// -----------------------------------------------------------------------------
`timescale 1ns / 1ps

module gemv_axi_resident #(
    parameter integer MMAX   = 1024,
    parameter integer KMAX   = 1024,
    parameter integer WWORDS = 200704,
        ...
        if (!aresetn) begin
            ...
            if (S_AXI_AWREADY) begin
                case (awaddr[5:2])
                    ...
                    4'hA: w_base <= S_AXI_WDATA[BAW-1:0];
                    default: ;
                endcase
        ...
        if (!aresetn) begin
            ...
            if (S_AXI_ARREADY) begin
                ...
                case (araddr_idx)
                    ...
                    4'h9: S_AXI_RDATA <= 32'h4745_4D52;       // "GEMR"
    ...
    gemv_resident #(.MMAX(MMAX), .KMAX(KMAX), .WWORDS(WWORDS), .RLAT(RLAT)) u_core (
