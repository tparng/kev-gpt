# Stage 2 step 2 — assemble the PS+PL block design for the LOADABLE GEMV engine
# (gemv_axi_load + gemv_load): one bitstream runs all 17 layers by streaming
# weights over AXI. No baked weights. Build-only; impl.tcl does synth/impl/bits.
#   vivado -mode batch -source fabric/stage2/tcl/build_bd_load.tcl
...
add_files -norecurse [list \
    "$root/fabric/stage2/rtl/gemv_load.sv" \
    "$root/fabric/stage2/rtl/gemv_axi_load.v"]
...
set g [create_bd_cell -type module -reference gemv_axi_load gemv]
set_property -dict [list CONFIG.MMAX {1024} CONFIG.KMAX {1024} \
    CONFIG.WBYTES {131072}] [get_bd_cells gemv]
