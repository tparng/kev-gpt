// -----------------------------------------------------------------------------
// layernorm_vec_gendiv — layernorm_vec.sv generalized to ANY D, not just a
// power of 2 (D=256 in the header math below is checkpoint C's own only
// tested case, purely illustrative -- the module is parametric).
//
// checkpoint C's layernorm_vec.sv (fabric/stage3/rtl/) computes mean/var as
// `sum >>> $clog2(D)` -- an exact arithmetic-shift divide by D, but ONLY
// correct when D is a power of 2 (fabric/stage3/run_layernorm.py's own
// _ln_int_quantized has an explicit assertion for exactly this, anticipating
// the case this file now covers). ASR's D=288 = 2^5*9 is NOT a power of 2 --
// gen2asr/ASR-ACCELERATOR-OP-SEQUENCE.md's Stage 2 table originally called
// LayerNorm reuse "REUSE, exact... only D=288 and per-layer gamma tables
// change, both already parametric" -- checked directly against the RTL while
// designing the decoder-block FSM, and that claim was WRONG: layernorm_vec.sv
// as-is silently computes sum>>>9 (i.e. /512) for D=288, not /288, a real
// ~1.78x error, not a rounding artifact.
//
// The ONLY change from layernorm_vec.sv: the two `>>> D_SH` divide sites
// (mean = sum/D, var = ssq/D) are replaced by an exact FLOOR integer divide
// (floordiv40/floordiv72 below) — round-toward-negative-infinity, matching
// `>>>`'s own semantics bit-for-bit for the power-of-2 case too, so this is a
// strict generalization: identical results to layernorm_vec.sv for D=256/128
// (regression-checked), and now also correct for D=288. Everything else
// (P-wide load, algebraic centered-sum identity, rsqrt seed+2-Newton,
// P-wide pipelined output) is copied verbatim, unchanged.
//
// y[i] = (x[i] - mean) * rsqrt(var + eps) * gamma[i]
// Formats (pinned, identical to layernorm_vec.sv):
//   x  signed Q6.25 (32b)   gamma signed Q4.20   mean=floor(sum/D) (Q6.25)
//   ssq = sum(xx) - 2*mean*sum + D*mean^2 (Q12.50)   var=floor(ssq/D)
//   A=(var>>>24)+eps (Q.26)   rsqrt -> Q.26     y = ((d*rsqrt)*gamma) >>> 49  (Q.22)
//
...
module layernorm_vec_gendiv #(
    parameter integer P = 8,                // vector lanes (D must be divisible by P)
    // ANY D here, not just a power of 2 (see file header) -- floordiv40/
    // floordiv72 do a real divide, not a shift.
    ...
    // ---- exact FLOOR integer divide by D (round toward -infinity), matching
    // `>>> $clog2(D)`'s own semantics bit-for-bit when D IS a power of 2 (so
    // this is a strict generalization, not a behavior change for that case).
    // Verilog's native `/` truncates toward zero; the sign-explicit rewrite
    // below converts that into floor division for D>0:
    //   num>=0: floor(num/D) == num/D (truncating == flooring for non-negative)
    //   num<0 : floor(num/D) == -ceil(-num/D) == -((-num + D-1)/D)  (truncating)
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_VAR: begin
                    mean  <= floordiv40(sum, D);                          // /D, Q6.25 (floor, real divide)
                    p_ct  <= $signed(floordiv40(sum, D)) * $signed(sum);       // mean*sum
                    p_mq  <= $signed(floordiv40(sum, D)) * $signed(floordiv40(sum, D)); // mean^2
                    state <= S_VAR2;
                end
                S_VAR2: begin
                ...
                S_MSB: begin
                    var_q <= floordiv72(ssq, D);                          // Q12.50 (floor, real divide)
                    A <= ($signed(floordiv72(ssq, D)) >>> (VAR_FRAC - A_FRAC)) + $signed(EPS_A);
...
endmodule // layernorm_vec_gendiv
