"""Sim gate for conv1d_seq.sv, parameterized as conv3 (+bias, real CIN=576
already P-aligned): real moonshine-tiny weights, real audio-derived
conv1+tanh+groupnorm1+conv2+gelu input, RTL checked bit-exact against
pack_conv3.py's own integer reference.

    python -m fabric.asr_seq.run_conv3

One sentinel line: CONV3_VERDICT.
"""
...
def run(sim_dir: str) -> bool:
    ...
    manifest = pack_conv3.main_gen(sim_dir)
    ...
    print(f"CONV3_VERDICT bitexact={ok} mismatches={mism}/{len(gold)} "
          f"TOUT={manifest['TOUT']} COUT={manifest['COUT']} got={len(got)}")
    return ok


def main(argv=None):
    ok = run(kevbuild("asr_conv3"))
