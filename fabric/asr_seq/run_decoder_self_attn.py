"""Sim gate for the decoder self-attention block: rope_apply_vec.sv (new) +
kv_bank.sv (checkpoint C's real RTL, unmodified) + vec_attn_w.sv (checkpoint
C's real RTL, unmodified), chained for real ASR data, checked bit-exact
against pack_decoder_self_attn.py's own golden reference.

    python -m fabric.asr_seq.run_decoder_self_attn

One sentinel line: DECODER_SELF_ATTN_VERDICT.
"""
...
def run(sim_dir: str) -> bool:
    ...
    return "DECODER_SELF_ATTN_VERDICT,bitexact=1" in rp.stdout


def main(argv=None):
    p = argparse.ArgumentParser(prog="fabric.asr_seq.run_decoder_self_attn")
    p.add_argument("--dir", default=kevbuild("asr_seq_selfattn"))
    a = p.parse_args(argv)
    ok = run(a.dir)
