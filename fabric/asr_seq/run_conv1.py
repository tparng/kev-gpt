"""Sim gate for conv1d_seq.sv, parameterized as conv1 -- Stage 1 conv
front-end's first (and structurally trickiest, CIN=1-padded-to-P) conv:
real moonshine-tiny weights, real audio (torch.manual_seed(0), L=3000),
RTL checked bit-exact against pack_conv1.py's own integer reference across
every real output position/channel.

    python -m fabric.asr_seq.run_conv1

One sentinel line: CONV1_VERDICT.
"""
...
def run(sim_dir: str) -> bool:
    ...
    manifest = pack_conv1.main_gen(sim_dir)
    ...
    print(f"CONV1_VERDICT bitexact={ok} mismatches={mism}/{len(gold)} "
          f"TOUT={manifest['TOUT']} COUT={manifest['COUT']} got={len(got)}")
    return ok


def main(argv=None):
    ok = run(kevbuild("asr_conv1"))
