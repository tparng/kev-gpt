"""vec_tanh RTL gate: bit-true vs the fixed-point reference, P lanes wide.

    python -m fabric.asr_seq.run_vec_tanh

vec_tanh (rtl/vec_tanh.sv) instantiates P independent copies of the
single-lane tanh_lut, so each lane must reproduce tanh_q
(fabric/asr_seq/run_tanh.tanh_q) EXACTLY. Same gate shape as
run_vec_silu.py/checkpoint C's run_vec_gelu.py. Format: I/O signed Q4.12
(16-bit, +-8).
"""
...
from fabric.asr_seq.run_tanh import tanh_q, tanh_table
...
def run(sim_dir, N=1024, P=8, seed=0):
    ...
    cp = subprocess.run(
        ["iverilog", "-g2012", "-o", vvp, f"-DN={N}", f"-DP={P}",
         os.path.join(HERE, "tb", "tb_vec_tanh.sv"),
         os.path.join(HERE, "rtl", "vec_tanh.sv"),
    ...
    print(f"VEC_TANH_VERDICT bitexact={ok} mismatches={mism}/{total} P={P}")
    return ok


def main(argv=None):
    sim_dir = kevbuild("asr_vec_tanh")
