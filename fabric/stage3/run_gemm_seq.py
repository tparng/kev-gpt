...
compared independently to seq_ref.full_forward_signals (tok + x4 + lnf + head).
    ...
    python -m fabric.stage3.run_gemm_seq --toks 48,10,100,77 --p 8 --lanes 128
"""
...
TB = os.path.join(HERE, "tb", "tb_seq_gemm.sv")
M64 = (1 << 64) - 1
...
def run(sim_dir, toks, P, lanes=128, tmax=32, npz="fabric/export/goformer.npz"):
    ...
    cp = subprocess.run(["iverilog", "-g2012", "-o", vvp,
                         *defs, f"-DPVAL={P}", f"-DNVAL={len(toks)}",
                         ...
                         TB,
                         os.path.join(RTL, "sequencer_gemm.sv"),
    ...
    try:
        ...
        pass
    n = len(toks)
    print(f"SEQ_GEMM_FULL N={n} P={P} L={lanes} TMAX={tmax} " + " ".join(parts) +
          f" ALL={all_ok} cyc_total={cyc} cyc/token={(cyc or 0)//n}")
    return all_ok


def main(argv=None):
    ap = argparse.ArgumentParser(prog="fabric.stage3.run_gemm_seq")
    ...
    ap.add_argument("--dir", default=kevbuild("stage3_seq_gemm"))
