# Stage 3 100k — synth + implement + bitstream for the SPLIT-BRAIN sequencer.
# Run AFTER build_bd_seq_sb.tcl. Emits gemv_seqsb.bit/.bin + util/timing.
#   vivado -mode batch -source impl_seq_sb.tcl [-tclargs <bdir>]
set bdir [lindex $argv 0]
if {$bdir eq ""} { set bdir "C:/kevbuild/stage3_seqsb_bit" }
open_project "$bdir/gemv_seqsb_pl/gemv_seqsb_pl.xpr"
...
set bit "$bdir/gemv_seqsb_pl/gemv_seqsb_pl.runs/impl_1/design_1_wrapper.bit"
if {[file exists $bit]} {
    file copy -force $bit "$bdir/gemv_seqsb.bit"
    write_cfgmem -force -format BIN -interface SMAPx32 -disablebitswap \
        -loadbit "up 0x0 $bit" "$bdir/gemv_seqsb" -quiet
    puts "BITSTREAM -> $bdir/gemv_seqsb.bit"
}
puts "IMPL_SB_DONE WNS=$wns"
