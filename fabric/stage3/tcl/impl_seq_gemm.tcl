...
# Run AFTER build_bd_seq_vec.tcl. Emits gemv_seqgemm.bit/.bin + util/timing.
set bdir "C:/kevbuild/stage3_seqgemm_bit"
open_project "$bdir/gemv_seqgemm_pl/gemv_seqgemm_pl.xpr"
...
set bit "$bdir/gemv_seqgemm_pl/gemv_seqgemm_pl.runs/impl_1/design_1_wrapper.bit"
if {[file exists $bit]} {
    file copy -force $bit "$bdir/gemv_seqgemm.bit"
    write_cfgmem -force -format BIN -interface SMAPx32 -disablebitswap \
        -loadbit "up 0x0 $bit" "$bdir/gemv_seqgemm" -quiet
    puts "BITSTREAM -> $bdir/gemv_seqgemm.bit"
}
puts "IMPL_GEMM_DONE WNS=$wns"
