# Doc-7 "Kevin remembers" — PS+PL block design for the KV-FAITHFUL sequencer
# (gemv_axi_seq_vec + sequencer_vec + kv_bank + vec_attn_w + layernorm_vec +
# vec_dequant + vec_gelu + gelu_lut/gelu_lut2 + softmax_f + gemv_banked_resident_vec +
# weight_bank_tdp). Build-only; impl_seq_kv.tcl does synth/impl/bitstream.
#   vivado -mode batch -source build_bd_seq_kv.tcl -tclargs <P> <LANES> <WWORDS> <FREQMHZ> <TMAX>
# PREREQUISITE: the .mem files in $mems (a run_vec_kv sim dir built with the SAME P —
# the wide-word ROMs are P-packed; run_vec_kv also emits the gelu_lut_e/o split and
# the kv_bank inv_lut_lo/hi ROMs that the plain run_vec_seq dirs do NOT have).
#
# GENERIC NOTES (checked against the RTL, 2026-06-10):
#  - TMAX: gemv_axi_seq_vec.v DOES forward TMAX to sequencer_vec (CONFIG.TMAX works).
#  - K2:   NOT a gemv_axi_seq_vec / sequencer_vec generic. sequencer_vec.sv pins
#          .K2(1) on its u_gemv (gemv_banked_resident_vec) instance — the value the
#          doc-7 build requires — so no CONFIG.K2 is set here. REQUIRED-WRAPPER-EDIT
#          only if K2 must ever differ from 1: expose K2 through gemv_axi_seq_vec.v
#          AND sequencer_vec.sv, then add CONFIG.K2 {1} to the set_property below.
set pp     [lindex $argv 0]
set lanes  [lindex $argv 1]
set wwords [lindex $argv 2]
set freq   [lindex $argv 3]
set tmax   [lindex $argv 4]
if {$pp     eq ""} { set pp 8 }
if {$lanes  eq ""} { set lanes 256 }
if {$wwords eq ""} { set wwords 16384 }
if {$freq   eq ""} { set freq 40 }
# TMAX=256 is the doc-7 KV window (kv_bank depth + pos-table depth).
if {$tmax   eq ""} { set tmax 256 }

set part  "xck26-sfvc784-2LV-c"
set board "xilinx.com:kv260_som:part0:1.4"
set root  [file normalize [file dirname [info script]]/../../..]
set bdir  "C:/kevbuild/stage3_seqkv_bit"
set mems  "C:/kevbuild/stage3_vec_kvk8t128"       ;# run_vec_kv sim dir — P-DEPENDENT (packed P/word)

file mkdir $bdir
create_project gemv_seqkv_pl "$bdir/gemv_seqkv_pl" -part $part -force
set_property board_part $board [current_project]

add_files -norecurse [list \
    "$root/fabric/stage3/rtl/gemv_axi_seq_vec.v" \
    "$root/fabric/stage3/rtl/sequencer_vec.sv" \
    "$root/fabric/stage3/rtl/kv_bank.sv" \
    "$root/fabric/stage3/rtl/vec_attn_w.sv" \
    "$root/fabric/stage3/rtl/layernorm_vec.sv" \
    "$root/fabric/stage3/rtl/vec_dequant.sv" \
    "$root/fabric/stage3/rtl/vec_gelu.sv" \
    "$root/fabric/stage3/rtl/gelu_lut.sv" \
    "$root/fabric/stage3/rtl/gelu_lut2.sv" \
    "$root/fabric/stage3/rtl/softmax_f.sv" \
    "$root/fabric/stage3/rtl/gemv_banked_resident_vec.sv" \
    "$root/fabric/stage3/rtl/weight_bank_tdp.sv"]

# wide-word BRAM-ROM init files (one P-packed word per line) + submodule LUTs.
# seed.mem feeds layernorm_vec's rsqrt seed ROM; gelu_lut_e/o the gelu_lut2 split
# (vec_gelu instantiates gelu_lut2, so plain gelu_lut.mem is NOT in the hierarchy);
# inv_lut_lo/hi the kv_bank dequant-inverse ROMs.
set memfiles [list tok_emb_w.mem pos_emb_w.mem gamma_w.mem dqm_w.mem dqe_w.mem \
                   inv_sact.mem seed.mem exp_lut.mem gelu_lut_e.mem gelu_lut_o.mem \
                   inv_lut_lo.mem inv_lut_hi.mem]
foreach mf $memfiles {
    # HARD ERROR, not a warning: a missing ROM init synthesises an all-zero LUT
    # (silent [Synth 8-4445]) — first silicon shipped with empty kv_bank inv ROMs
    # this way and quantised every KV code to 0 (log §38).
    if {![file exists "$mems/$mf"]} { error "missing required ROM init $mems/$mf" }
    add_files -norecurse "$mems/$mf"
    set_property file_type {Memory Initialization Files} [get_files "$mems/$mf"]
}
update_compile_order -fileset sources_1

create_bd_design design_1
set ps [create_bd_cell -type ip -vlnv xilinx.com:ip:zynq_ultra_ps_e:* ps]
apply_bd_automation -rule xilinx.com:bd_rule:zynq_ultra_ps_e -config {apply_board_preset 1} [get_bd_cells ps]
set_property -dict [list \
    CONFIG.PSU__USE__M_AXI_GP0 {1} CONFIG.PSU__USE__M_AXI_GP1 {0} \
    CONFIG.PSU__USE__M_AXI_GP2 {0} CONFIG.PSU__FPGA_PL0_ENABLE {1} \
    CONFIG.PSU__CRL_APB__PL0_REF_CTRL__FREQMHZ $freq] [get_bd_cells ps]

set g [create_bd_cell -type module -reference gemv_axi_seq_vec seq]
set_property -dict [list CONFIG.P $pp CONFIG.LANES $lanes CONFIG.NLAYER {4} \
    CONFIG.WWORDS $wwords CONFIG.TMAX $tmax CONFIG.C_S_AXI_ADDR_WIDTH {8}] [get_bd_cells seq]

apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [list \
    Clk_master {Auto} Clk_slave {Auto} Clk_xbar {Auto} \
    Master {/ps/M_AXI_HPM0_FPD} Slave {/seq/S_AXI} \
    intc_ip {New AXI SmartConnect} master_apm {0}] [get_bd_intf_pins /seq/S_AXI]

assign_bd_address
catch { set_property offset 0xA0000000 [get_bd_addr_segs */seq/*] }
regenerate_bd_layout
save_bd_design
validate_bd_design
make_wrapper -files [get_files design_1.bd] -top -import
set_property top design_1_wrapper [current_fileset]
generate_target all [get_files design_1.bd]
update_compile_order -fileset sources_1
puts "BD_BUILD_KV_OK p=$pp lanes=$lanes wwords=$wwords freq=$freq tmax=$tmax"
