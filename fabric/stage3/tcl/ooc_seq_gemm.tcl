# OOC fit/Fmax check for sequencer_gemm (the P-wide datapath sequencer) at a chosen P/LANES.
...
#   vivado -mode batch -source ooc_seq_gemm.tcl -tclargs <P> <LANES> <PERIOD_NS>
...
synth_design -top sequencer_gemm -part $part -mode out_of_context \
    -generic P=$pp -generic LANES=$lanes -generic N=$nn -generic NLAYER=4 \
    -generic WWORDS=$wwords -generic TMAX=$tmax

create_clock -name clk -period $period [get_ports clk]

puts "===== OOC sequencer_gemm P=$pp LANES=$lanes @ ${period}ns ====="
report_utilization
puts "----- timing -----"
