...
// =============================================================================
// sequencer_fast — RESIDENT-READ variant of sequencer.sv (THE LEAP, step 1).
// Identical control + datapath to sequencer.sv EXCEPT the GEMV core is
// gemv_banked_resident: the whole INT4 image preloads ONCE into the core's URAM,
// and each matmul just sets w_base (the layer's word offset) + streams activations.
// This DELETES the per-matmul weight-reload loop (the old G_LDADDR/G_LDW), which was
// ~half the cycle budget. Bit-exact to sequencer.sv by construction (same transposed
// math, same resident layout/offsets) — gated vs seq_ref token stream. LANES is free
// to widen (PE=256): the readback interface is lane-agnostic and GW_* scale with LANES.
// =============================================================================
module sequencer_fast #(
    ...
    // ---- WEIGHT IMAGE now lives INSIDE the resident GEMV core (u_gemv) ------------------
    // The host streams the whole transposed INT4 image ONCE through wl_* straight into the
    // core's resident URAM (wl_rst/wl_we/wl_data routed to the core's ld_rst/w_we/w_data at
    // the instantiation). There is NO separate weight URAM and NO per-matmul copy: the FSM
    // just sets w_base for each matmul and gemv_banked_resident reads the weights in place.
    // This deletes the old G_LDADDR/G_LDW reload loop — roughly half the per-token cycles.
    ...
    reg [10:0]         gv_m, gv_k;
    reg [WAW-1:0]      gv_wbase;     // resident word offset of the CURRENT matmul
    wire               gv_done;
    reg [9:0]          gv_rdaddr;
    wire signed [31:0] gv_yout;
    // RESIDENT GEMV: the WHOLE transposed INT4 image is preloaded ONCE into u_gemv's URAM
    // via the host wl_* stream (wl_we = one 32-bit chunk, low-first; assembled internally).
    // WWORDS = WROM_N (the exact image word count). Per matmul the FSM sets gv_wbase (the
    // layer's word offset) + M/K, streams the activation, and runs — NO per-matmul weight
    // reload (the old G_LDADDR/G_LDW loop is gone). ld_rst is shared: wl_rst rewinds the
    // weight write pointer at boot; gv_ldrst rewinds the activation pointer before each
    // matmul (it also zeroes the weight write pointer, which is harmless post-boot since the
    // FSM never asserts w_we — only the host does, once).
    gemv_banked_resident #(.LANES(LANES), .MMAX(D_MLP), .KMAX(D_MLP), .RLAT(2),
                  .WWORDS(WROM_N)) u_gemv (
...
`endif
    ...
    always @(posedge clk) begin
        gv_ldrst<=0; gv_xwe<=0; gv_start<=0;
...
`endif
        ...
        // =========================================================
        // RESIDENT weights: the whole transposed image is already in u_gemv's URAM (loaded
        // once via wl_*). There is NO per-matmul weight stream. G_LDRST just pulses the
        // core's ld_rst to rewind the activation write pointer, then we stream this matmul's
        // acts in G_ACT. The layer's w_base is latched at G_RUN. (G_LDADDR/G_LDW are gone.)
        G_LDRST:  begin gv_ldrst<=1; ci<=0; st<=G_ACT; end
        G_ACT: begin
        ...
        G_RUN: begin gv_m<=gM[10:0]; gv_k<=gK[10:0];
                     gv_wbase<=wbase[WAW-1:0]; gv_start<=1; st<=G_WAIT; end
