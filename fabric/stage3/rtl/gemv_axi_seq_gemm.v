// -----------------------------------------------------------------------------
// gemv_axi_seq_gemm — AXI4-Lite shell around sequencer_gemm (the N-stream batch
// sequencer). Same skeleton as gemv_axi_seq_vec.v. IDCODE "SQGM" (0x5351474D).
//
...
module gemv_axi_seq_gemm #(
        ...
        if (!aresetn) begin
            ...
            if (S_AXI_AWREADY) begin
                case (awaddr[7:2])
                    ...
                    6'h12: rd_stream <= S_AXI_WDATA[1:0];
        ...
        if (!aresetn) begin
            ...
            if (S_AXI_ARREADY) begin
                ...
                case (araddr_idx)
                    ...
                    6'h0B: S_AXI_RDATA <= 32'h5351_474D;          // "SQGM"
    ...
    sequencer_gemm #(.P(P), .LANES(LANES), .N(N), .NLAYER(NLAYER), .WWORDS(WWORDS),
                     .TMAX(TMAX)) u_seq (
        .clk(clk), .rst(core_rst), .go(go_pulse),
        .tok_ids({tok_id3, tok_id2, tok_id1, tok_id0}), .pos(pos),
