        ...
        if (rst) begin
            ...
            case (state)
                ...
                DRAIN: begin                  // one stream's group word per cycle
                    // constant-base part-selects only — a variable 4096-bit +:
                    // is exactly the over-wide-X iverilog trap.
                    case (db[1:0])
                        2'd0: acc_sel = acc_cat[0*YBITS +: YBITS];
                        2'd1: acc_sel = acc_cat[(N > 1 ? 1 : 0)*YBITS +: YBITS];
                        2'd2: acc_sel = acc_cat[(N > 2 ? 2 : 0)*YBITS +: YBITS];
                        default: acc_sel = acc_cat[(N > 3 ? 3 : 0)*YBITS +: YBITS];
                    endcase
