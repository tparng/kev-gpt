// -----------------------------------------------------------------------------
// gemv_axi_seq_pp — AXI4-Lite shell around sequencer_pp (the N-stream batch
// sequencer). Same skeleton as gemv_axi_seq_vec.v. IDCODE "SQPP" (0x53515050).
//
// Flow: CTRL.wl_rst, stream the transposed INT4 image to W_DATA; write the four
// TOK_ID registers; CTRL.go; poll STATUS.done; read four TOK_OUT registers +
// CYCLES (go..done covers ALL N streams — aggregate tok/s = N*fclk/CYCLES).
//
...
//  0x30 TOK_ID1  0x34 TOK_ID2  0x38 TOK_ID3   (stream 1..3 inputs)
//  0x3C TOK_OUT1  0x40 TOK_OUT2  0x44 TOK_OUT3 (stream 1..3 outputs)
//  0x48 RD_STREAM (bank readback stream select)
//  0x4C E_DATA (el_we: embed-table load, streamed after the weight image)
// -----------------------------------------------------------------------------
`timescale 1ns / 1ps

module gemv_axi_seq_pp #(
    ...
    reg [1:0]    dbg_stop;
    reg [31:0]   wl_data;
    reg [8:0]    tok_id0, tok_id1, tok_id2, tok_id3, pos;
    reg [8:0]    tok_id4, tok_id5, tok_id6, tok_id7;
    reg [3:0]    rd_sel;
    reg [10:0]   rd_addr;
    reg [2:0]    rd_stream;
    ...
    always @(posedge clk) begin
        if (!aresetn) begin
            ...
            tok_id0<=0; tok_id1<=0; tok_id2<=0; tok_id3<=0; pos<=0;
            tok_id4<=0; tok_id5<=0; tok_id6<=0; tok_id7<=0;
            ...
            if (S_AXI_AWREADY) begin
                case (awaddr[7:2])
                    ...
                    6'h0C: tok_id1 <= S_AXI_WDATA[8:0];
                    6'h0D: tok_id2 <= S_AXI_WDATA[8:0];
                    6'h0E: tok_id3 <= S_AXI_WDATA[8:0];
                    6'h12: rd_stream <= S_AXI_WDATA[2:0];
                    6'h13: begin el_we<=1; wl_data<=S_AXI_WDATA; end
                    6'h14: tok_id4 <= S_AXI_WDATA[8:0];     // 0x50
                    6'h15: tok_id5 <= S_AXI_WDATA[8:0];     // 0x54
                    6'h16: tok_id6 <= S_AXI_WDATA[8:0];     // 0x58
                    6'h17: tok_id7 <= S_AXI_WDATA[8:0];     // 0x5C
                    default: ;
                endcase
        ...
        if (!aresetn) begin
            ...
            if (S_AXI_ARREADY) begin
                ...
                case (araddr_idx)
                    ...
                    6'h0B: S_AXI_RDATA <= 32'h5351_5050;          // "SQGM"
    ...
    sequencer_pp #(.P(P), .LANES(LANES), .N(N), .NLAYER(NLAYER), .WWORDS(WWORDS),
                     .TMAX(TMAX)) u_seq (
        .clk(clk), .rst(core_rst), .go(go_pulse),
        .tok_ids({tok_id7, tok_id6, tok_id5, tok_id4, tok_id3, tok_id2, tok_id1, tok_id0}),
        .pos(pos),
