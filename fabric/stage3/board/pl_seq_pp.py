...
def main(argv=None):
    ...
    try:
        ...
        if match and cyc > 0:
            ...
            print(f"PL_SEQPP_VERDICT match=True cyc={cyc} agg_tok_s={agg:.1f} "
                  f"fclk={fclk_hz:.0f} idcode=0x{idc:08X}")
        else:
            print(f"PL_SEQPP_VERDICT match={match} cyc={cyc} (tok/s WITHHELD unless match) "
                  f"idcode=0x{idc:08X}")
