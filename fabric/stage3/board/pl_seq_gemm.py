...
R_TOKID = [0x08, 0x30, 0x34, 0x38]
R_TOKOUT = [0x24, 0x3C, 0x40, 0x44]
R_RDSTREAM = 0x48
R_EDATA = 0x4C
TMAX = 32
RESID_FRAC = 25
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="N=4 batch GEMM sequencer on silicon")
    ap.add_argument("--npz", default=os.path.join(_REPO, "fabric", "export", "goformer.npz"))
    ap.add_argument("--lanes", type=int, default=128)
    ap.add_argument("--toks", default="48,10,100,77")
    ...
    try:
        idc = dev.rd(R_IDCODE)
        print(f"[idc ] 0x{idc:08X} {'OK' if idc == IDCODE_SQGM else 'MISMATCH (expect SQGM)'}")
        if idc != IDCODE_SQGM:
            return 3
        ...
        if match and cyc > 0:
            ...
            print(f"PL_SEQGEMM_VERDICT match=True cyc={cyc} agg_tok_s={agg:.1f} "
                  f"fclk={fclk_hz:.0f} idcode=0x{idc:08X}")
        else:
            print(f"PL_SEQGEMM_VERDICT match={match} cyc={cyc} (tok/s WITHHELD unless match) "
                  f"idcode=0x{idc:08X}")
