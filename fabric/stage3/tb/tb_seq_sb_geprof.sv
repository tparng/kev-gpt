...
module tb;
    ...
    integer gi;
    // ---- vec_attn shared-unit per-state residency (observer only) ----
    //   0 IDLE 1 LD_Q 2 LD_K 3 LD_V 4 SCORE 5 SCORE_EMIT 6 SM_COLL
    //   7 CTX  8 CTX_EMIT 9 DONE
    integer at_st_cnt [0:9];
    // attn-busy split: cycles the unit is busy (st!=IDLE) vs idle while SOME cohort
    // wants it (at_req but unit IDLE -> pure handoff latency)
    integer at_busy_cyc, at_idle_want_cyc;
    // softmax sub-residency inside the shared attn (deep): the divider + 3 passes
    //   0 IDLE 1 LOAD 2 EXP 3 RECIP 4 NORM 5 DONE
    integer sm_st_cnt [0:5];
    initial begin
        ...
        for (gi=0; gi<10; gi=gi+1) at_st_cnt[gi]=0;
        for (gi=0; gi<6;  gi=gi+1) sm_st_cnt[gi]=0;
        at_busy_cyc=0; at_idle_want_cyc=0;
    end
    always @(posedge clk) begin
        ...
        if (running && !done) begin
            ...
            // ATT2=0: attention is shared; the per-cohort attn instrumentation
            // is disabled in this GE-focused profiler variant.
        end
        if (dbgcyc % 100000 == 0)
    ...
    task prof_report;
        ...
        begin
            ...
            $display("AT_LD_V       %8d", at_st_cnt[3]);
            $display("AT_SCORE      %8d", at_st_cnt[4]);
            $display("AT_SCORE_EMIT %8d", at_st_cnt[5]);
            $display("AT_SM_COLL    %8d", at_st_cnt[6]);
            $display("AT_CTX        %8d", at_st_cnt[7]);
            $display("AT_CTX_EMIT   %8d", at_st_cnt[8]);
