...
# 6. start the chat server (serves client.html + the WS on one port). Pick a backend:
#
#    (a) stub — telegraphic gibberish, no model; exercises the whole plumbing:
python -m webchat.demo.server --backend stub --port 8090
#
#    (b) REAL Kevin, single box (model runs in the server process). numpy off-box,
#        or pl/c when the server runs ON the Kria with the resident bitstream:
python -m webchat.demo.server --backend kv --kv-backend numpy --gen-chars 24 --port 8090
#
#    (c) the production SPLIT — Precision serves, Kria has the fabric, reached over
#        Tailscale/GigE via the A53 daemon:
python -m webchat.demo.server --backend tcp --port 8090 \
    --daemon-host <kria-ip> --daemon-port 9099 --daemon-transport json
#        ...and on the KRIA (chat bitstream loaded, weights boot-streamed):
#          REAL multi-token Kevin (host-KV decoder, GEMVs on the PL):
#            sudo python -m webchat.demo.a53_daemon --port 9099 --json \
#                 --engine kv --kv-backend c --gen-chars 24
#          or first-token-faithful single-token (the 60k pass): --engine t1 (default)
#
#    NOTES: --daemon-transport MUST match the daemon (pass --json on BOTH). The kv
#    engine is the model-faithful MULTI-TOKEN path (real text, heterogeneous rate);
#    --engine t1 / --backend pl is the single-token 60k pass (first token faithful,
#    rest is plumbing). A fully on-chip multi-token decode at fabric speed is the
#    unbuilt P1 RTL (5-demo-prd.md gap 1).
